...
export function WhatsAppLinkModal({
  ...
  useEffect(() => {
    ...
    async function generateToken() {
      ...
      setError(null);
      try {
        const res = await proxyFetch("/api/whatsapp/link-token", {
          method: "POST",
        });
        if (!res.ok) {
  ...
  useSWR<LinkStatusResponse>(
    shouldPoll ? "/api/whatsapp/link-status" : null,
    linkStatusFetcher,
    {
      ...
      <div className="relative surface rounded-xl max-w-md w-full p-8 bg-bg-surface">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-lg bg-bg-elevated ring-1 ring-border mb-4">
            <ServiceLogo name="whatsapp" size={28} />
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-text-secondary leading-relaxed">
                Busca el boton de chat nuevo en WhatsApp y escribe el numero del bot de KillaAssistant que te indicaron.
              </p>
            </div>
